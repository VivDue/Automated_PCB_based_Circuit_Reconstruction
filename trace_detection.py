...
class TraceDetection:
    ...
    def create_net_list(self, masks, mirror = [False, True])->list:
        ...
        # print the len of the net list
        print(f"Number of nets: {len(net_list)}")

        # Group the net list
        image = masks["inpainted"][0]
        grouped_net_list = self._group_net_list(image,net_list)

        # print the grouped net list
        for i, group in enumerate(grouped_net_list):
            print(f"Group {i}:")
            for net in group:
                print(net)
    ...
    def _add_border(self, mask, border_size=10):
        # Füge einen weißen Rand um die Maske hinzu (border_size gibt die Dicke des Randes an)
    ...
    def _find_contours_without_border(self, mask, border_size=10):
        ...
        # Berechne die Konturen auf der gepaddeten Maske
        contours, hierarchy = cv2.findContours(padded_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        
        # Entferne den Offset durch den Rahmen von den Konturen
...
####################################################################################################
    ...
    def _mask_silkscreen(self, layer_image, background_mask):
        ...
        # TODO remove pads and background
    ...
    def _mask_traces(self, inpainted_image, pad_mask, background_mask):
        ...
        # Morphological operations to remove noise
        kernel = np.ones((3,3),np.uint8)
        trace_mask = cv2.erode(result_image, kernel, iterations=1)
